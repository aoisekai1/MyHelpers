...
function FormatDate(date = null, formatDate = 'Y-m-d', obj={}) {
    ...
    if (date == "first") {
        date = new Date(myDate.getFullYear(), myDate.getMonth(), 1);
    }
    if (date == "last") {
        date = new Date(myDate.getFullYear(), myDate.getMonth() + 1, 0);
    }
 ...
 *  @param symbolSparator String
 * 
 * */
function FormatCurrency(numberCurrency = 0, obj = {}) {
    let currency = '';
    ...
    if (!obj.symbolSparator) {
        obj.symbolSparator = (toLowerString == 'idr' || toLowerString == 'myr') ? '.' : ',';
    }
    //Show code currency when showCurrencyType is true
    let symbolCurrency = obj.showCurrencyType ? currencyType : '';
    let currDec = '';
    //Check and show symbol currency
    if (isNotEmpty && obj.showSymbolCurrency) {
        symbolCurrency = obj.showSymbolCurrency ? resCurrency[0].symbol : resCurrency[0].currencyType.toLocaleUpperCase();
    }
    if (obj.symbolSparator == "." && obj.showDec) {
        currDec = ",";
    }
    if (obj.symbolSparator == "," && obj.showDec) {
        currDec = ".";
    ...
    if (obj.showDec) {
        currDec = obj.showDec ? resCurrency[0].currencyType == 'idr' || resCurrency[0].currencyType == 'myr' ? ',00' : '.00' : ''
    }
    ...
    for (let x in numSplit) {
        if (!(x % 3) && x != 0) {
            arrNum.push(currNum);
            arrNum.push(obj.symbolSparator);
        } else {
            arrNum.push(currNum);
        }
